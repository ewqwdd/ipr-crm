...
export default function AddSpecModal({ open, name, value, setValue, setOpen, onSubmit, loading }: AddSpecModalProps) {
  const { data, isFetching } = universalApi.useGetSpecsQuery()

  const options = data?.map((spec) => ({ id: spec.id, name: spec.name })) ?? []

  return (
    <Modal open={!!open} setOpen={setOpen} onSubmit={onSubmit} loading={loading}>
      <Heading title="Специализация" description="Вы можете заменить или добавить специализацию" />
      <span className="font-semibold text-gray-700 text-sm my-6">{name}</span>
      <SearchSelectMultiple loading={isFetching} options={options} value={value} setValue={setValue} />
    </Modal>
  )
}
