...
export default function SearchSelectMultiple({ setValue, value = [], options, loading, label }: SearchSelectMultipleProps) {
  ...
  return (
    ...
    <Combobox as="div" value={value} onChange={handleSelect} className={cva({ 'animate-pulse': !!loading })}>
          ...
          <Combobox.Options className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
            {filteredData.map((option) => (
              ...
              >
                {({ active }) => (
                  <>
                    <span className={classNames('block truncate', value.some((item) => item.id === option.id) && 'font-semibold')}>
                      {option.name}
                    </span>
