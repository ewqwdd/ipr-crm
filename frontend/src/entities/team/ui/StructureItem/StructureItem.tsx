...
export default memo(function StructureItem({ team, openModal, startEditing, current, openDeleteModal }: StructureItemProps) {
  ...
  const title = (
    <>
      ...
      {openDeleteModal && (
        <button className="ml-auto mr-4" onClick={(e) => openDeleteModal(e, team)}>
  ...
  return (
    ...
    <div onClick={onClick}>
      <Accordion
        btnClassName={cva('bg-gray-100 hover:bg-gray-200 p-2 pb-3 border-b border-b-gray-300 [&>button]:flex-none cursor-pointer', {
          'bg-gray-200': current === team.id,
        })}
        title={<div className="flex items-center flex-1">{title}</div>}
        className="pt-0"
      >
        <div className="pl-3">{team.subTeams?.map((child) => <StructureItem openDeleteModal={openDeleteModal} openModal={openModal} current={current} startEditing={startEditing} key={child.id} team={child} />)}</div>
      </Accordion>
    </div>
